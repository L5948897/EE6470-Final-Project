...
{
    //HLS_MAP_TO_REG_BANK( RoundKey );
    //HLS_MAP_TO_REG_BANK( SBox );
	SC_THREAD( do_filter );
	sensitive << i_clk.pos();
	dont_initialize();
...
#ifndef NATIVE_SYSTEMC
	i_plaintext1.clk_rst(i_clk, i_rst);
	i_plaintext2.clk_rst(i_clk, i_rst);
	i_key1.clk_rst(i_clk, i_rst);
	i_key2.clk_rst(i_clk, i_rst);
  	o_ciphertext1.clk_rst(i_clk, i_rst);
	o_ciphertext2.clk_rst(i_clk, i_rst);
#endif
}
...
// SBox values
const uint8_t SobelFilter::SBox[256] = {
...
void SobelFilter::KeyExpansion(sc_dt::sc_uint<64>& key1, sc_dt::sc_uint<64>& key2) { //check
    for (int i = 0; i < 16; ++i) {
        HLS_CONSTRAIN_LATENCY(0, 1, "lat_keyexp_init");
		if (i < 8){
...
key_loop:
    while (i < 176) {
        for (int j = 0; j < 4; ++j) {
            HLS_CONSTRAIN_LATENCY(0, 2, "lat_keyexp_temp");
            temp[j] = RoundKey[i - 4 + j];
        }
        if (i % 16 == 0) {
            HLS_CONSTRAIN_LATENCY(0, 10, "lat_keyexp_rot");
        ...
        for (int j = 0; j < 4; ++j) {
            HLS_CONSTRAIN_LATENCY(0, 2, "lat_keyexp_assign");
...
void SobelFilter::AddRoundKey(sc_dt::sc_uint<64>& state1, sc_dt::sc_uint<64>& state2, int round) { //check
    for (int i = 0; i < 16; ++i) {
        HLS_CONSTRAIN_LATENCY(0, 1, "lat_addroundkey");
		if (i < 8){
...
void SobelFilter::SubBytes(sc_dt::sc_uint<64>& state1, sc_dt::sc_uint<64>& state2) { //check
    for (int i = 0; i < 8; ++i) {
        HLS_CONSTRAIN_LATENCY(0, 2, "lat_subbytes");
...
void SobelFilter::ShiftRows(sc_dt::sc_uint<64>& state1, sc_dt::sc_uint<64>& state2) { //check
    sc_dt::sc_uint<64> temp1, temp2;
    HLS_CONSTRAIN_LATENCY(0, 1, "lat_shiftrows_init");
    // First row (no shift)
...
void SobelFilter::MixColumns(sc_dt::sc_uint<64>& state1, sc_dt::sc_uint<64>& state2) { //check
    sc_dt::sc_uint<8> a, b, c, d;
	for (int i = 0; i < 4; ++i) {
        HLS_CONSTRAIN_LATENCY(0, 1, "lat_mixcolumns");
		if (i < 2){
...
sc_dt::sc_uint<8> SobelFilter::GfMul(sc_dt::sc_uint<8> a, sc_dt::sc_uint<8> b) {
    sc_dt::sc_uint<8> p = 0;
...
/*#if defined (II)
        ...
        for (int i = 0; i < 10; i++) {
...
#ifndef NATIVE_SYSTEMC
            {
                ...
                plaintext1 = i_plaintext1.get();
                plaintext2 = i_plaintext2.get();
                wait();
                key1 = i_key1.get();
                key2 = i_key2.get();
                wait();
...
#else
			plaintext1 = i_plaintext1.read();
			plaintext2 = i_plaintext2.read();
			key1 = i_key1.read();
			key2 = i_key2.read();
#endif
            ...
            HLS_CONSTRAIN_LATENCY(0, 31, "all_lat");
            //encryption
...
main_loop:
...
#ifndef NATIVE_SYSTEMC
            {
                ...
                o_ciphertext1.put(state1);
                o_ciphertext2.put(state2);
                wait();
            }
#else
            o_ciphertext1.write(state1);
            o_ciphertext2.write(state2);
#endif
        }
	}
}
