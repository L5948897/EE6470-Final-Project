...
static char* const SOBELFILTER_START_ADDR1 = reinterpret_cast<char* const>(0x73001000);
static char* const SOBELFILTER_READ_ADDR1  = reinterpret_cast<char* const>(0x73001004);
...
int main(unsigned hart_id) {
  ...
  uint64_t plaintext1[10] = { 0x3243f6a8885a308d,
                              0x0011223344556677,
                              0x0001020304050607,
                              0x0123456789abcdef,
                              0xffeeddccbbaa9988,
                              ...
                              0x0000000000000000,
                              0x8976546556131654};
  uint64_t plaintext2[10] = { 0x313198a2e0370734,
                              0x8899aabbccddeeff,
                              0x08090a0b0c0d0e0f,
                              0x0123456789abcdef,
                              0x7766554433221100,
  ...
  for (int i = begin_idx; i < end_idx; i++){
    ...
    for (int k = 0; k < 2; k++){
      if (k == 0){
        ...
        for (int j = 0; j < 4; j++){ //send key
          ...
          if (j < 2){
            buffer.byte[0] = key1>>((1-j)*32+24);
            buffer.byte[1] = key1>>((1-j)*32+16);
            buffer.byte[2] = key1>>((1-j)*32+8);
            buffer.byte[3] = key1>>((1-j)*32);
          } else {
            buffer.byte[0] = key2>>((3-j)*32+24);
            buffer.byte[1] = key2>>((3-j)*32+16);
            buffer.byte[2] = key2>>((3-j)*32+8);
            buffer.byte[3] = key2>>((3-j)*32);
          }
          
          sem_wait(&lock);
          if (hart_id == 0){
            write_data_to_ACC(SOBELFILTER_START_ADDR0, buffer.uc, 4);
          } else {
            write_data_to_ACC(SOBELFILTER_START_ADDR1, buffer.uc, 4);
          }
          sem_post(&lock);
        }
      } else {
        ...
        for (int j = 0; j < 4; j++){ //send plaintext
          ...
          if (j < 2){
            buffer.byte[0] = plaintext1[i]>>((1-j)*32+24);
            buffer.byte[1] = plaintext1[i]>>((1-j)*32+16);
            buffer.byte[2] = plaintext1[i]>>((1-j)*32+8);
            buffer.byte[3] = plaintext1[i]>>((1-j)*32);
          } else {
            buffer.byte[0] = plaintext2[i]>>((3-j)*32+24);
            buffer.byte[1] = plaintext2[i]>>((3-j)*32+16);
            buffer.byte[2] = plaintext2[i]>>((3-j)*32+8);
            buffer.byte[3] = plaintext2[i]>>((3-j)*32);
          }
    ...
    while(c < 6){
      sem_wait(&lock);
      if(hart_id == 0){
        read_data_from_ACC(SOBELFILTER_READ_ADDR0, buffer.uc, 4);
      } else {
        read_data_from_ACC(SOBELFILTER_READ_ADDR1, buffer.uc, 4);
      }
      sem_post(&lock);
      memcpy(data.uc, buffer.uc, 4);
      //std::cout << "Received data = " << std::hex << data.uint << std::endl;
  ...
  if (hart_id == 0){
    ...
    for (int i = 0; i < TEXT; i++){
      ...
      printf("Ciphertext1 = %x\n", ciphertext1[i]);
      printf("Ciphertext2 = %x\n", ciphertext2[i]);
